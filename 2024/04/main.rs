...
pub fn part_one(input: &str) -> usize {
  ...
  letter_grid.iter()
    ...
    .flat_map(|(start, _)|
      [Vec2::new(1,0), Vec2::new(0,1), Vec2::new(1,1), Vec2::new(1,-1)].into_iter()
...
pub fn part_two(input: &str) -> usize {
  ...
  letter_grid.iter()
    .filter(|(_,ch)| **ch == 'A')
    .filter(|(start, _)| {
      let first = [Vec2::new(-1,-1),Vec2::new(0,0), Vec2::new(1,1)].into_iter().map(|step| **start + step).filter_map(|vec| letter_grid.get(&vec)).collect::<String>();
      if first != "MAS" && first != "SAM" {
        false
      } else {
        let second = [Vec2::new(-1,1),Vec2::new(0,0), Vec2::new(1,-1)].into_iter().map(|step| **start + step).filter_map(|vec| letter_grid.get(&vec)).collect::<String>();
