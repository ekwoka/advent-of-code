...
pub fn part_one(input: &str) -> usize {
  ...
  (0..plots.len()).flat_map(|y| (0..plots[0].len()).map(move |x| (y,x)))
    .filter_map(|coord| {
...
pub fn part_two(input: &str) -> usize {
  ...
  (0..plots.len()).flat_map(|y| (0..plots[0].len()).map(move |x| (y,x)))
    .filter_map(|coord| {
      ...
      let mut edges = contained.clone().iter()
      .flat_map(|(y,x)| {
        let mut sides = Vec::<(usize,usize)>::new();
        if *x == 0
        || !contained.contains(&(*y,x-1)) {
          sides.push((*y,x*2));
        }
        if !contained.contains(&(*y,x+1)) {
          sides.push((*y,(x*2+1)));
      ...
      edges.iter().for_each(|(y, x)| {
        let existing_idx = vert_sides.iter().position(|side| side.0 == *x && (side.1 == y+1 || side.2 == *y-1));
...
mod tests {
  ...
  use test::Bencher;
    ...
    fn part_one_bench(b: &mut Bencher) {
        let input = include_str!("../../utils/.cache/2024-12.txt").trim();
