...
pub fn part_two(input: &str) -> u64 {
    ...
    while marker > 0 {
        if let Some(id) = blocks[marker] {
            let mut size = 1i32;
            while marker > 0
                && let Some(next_id) = blocks[marker - 1]
            {
                if next_id == id {
                    size += 1;
                    marker -= 1;
            ...
            while empty_marker < marker {
                while blocks[empty_marker].is_some() {
                    empty_marker += 1;
                }
                let mut empty_size = 1;
                while empty_marker + 1 < blocks.len()
                    && let None = blocks[empty_marker + 1]
                {
                    empty_size += 1;
                    empty_marker += 1;
                }
