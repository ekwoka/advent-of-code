...
impl GateWire {
    ...
    fn has_broken(&self, name: &str, wires: &HashMap<String, GateWire>) -> Option<String> {
        ...
        match self {
            ...
            GateWire::Xor(a, b) => {
                if let Some(GateWire::Xor(_, _)) = wires.get(a)
                    && let Some(GateWire::Or(_, _)) = wires.get(b)
                {
                    Some(name.to_string())
                } else if let Some(GateWire::Or(_, _)) = wires.get(a)
                    && let Some(GateWire::Xor(_, _)) = wires.get(b)
                {
                    Some(name.to_string())
                } else {
                    None
            ...
            GateWire::Or(a, b) => match wires.get(a) {
...
mod tests {
    ...
    fn part_one_bench(b: &mut Bencher) {
        let input = include_str!("../../node_modules/.aoc-cache/2024-24.txt").trim();
    ...
    fn part_two_bench(b: &mut Bencher) {
        ...
        b.iter(move || {
            assert_eq!(
                part_two(input),
                "gfv,hcm,kfs,tqm,vwr,z06,z11,z16".to_string()
            )
        });
    }
}
