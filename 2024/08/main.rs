    ...
    let antennae = input
        ...
        .flat_map(|(y, line)| {
            line.chars()
                .enumerate()
                .filter(|(_, ch)| ch != &'.')
                .map(move |(x, ch)| Antenna(ch.to_owned(), Vec2::new(x as i32, y as i32)))
        })
        .collect::<Vec<_>>();
    antennae
        .iter()
        ...
        .filter(|antinode| {
            antinode.x >= 0 && antinode.x < width && antinode.y >= 0 && antinode.y < height
        })
        .collect::<HashSet<_>>()
        .len()
}
...
pub fn part_two(input: &str) -> usize {
    ...
    antennae
        .iter()
        .flat_map(|antenna| {
            antennae.iter().filter_map(move |other| {
                if *other != *antenna && antenna.same_frequency(other) {
        ...
        .flat_map(|(position, offset)| {
            let low = -std::cmp::max(position.x / offset.x.abs(), position.y / offset.y.abs());
            let high = std::cmp::max(
                (width - position.x) / offset.x.abs(),
                (height - position.y) / offset.y.abs(),
            );
