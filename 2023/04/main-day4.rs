    ...
    input
        ...
        .map(|card| {
            ...
            let winning_numbers = number_regex
                ...
                .collect::<Vec<i32>>();
            number_regex
                .captures_iter(card[1])
                .map(|cap| cap[0].parse::<i32>().unwrap())
                .filter(|number| winning_numbers.contains(number))
                .fold(0, |acc, _number| if acc == 0 { 1 } else { acc * 2 })
        })
        .sum()
}
...
pub extern "C" fn part_two(cstring: *const c_char) -> i32 {
    ...
    let card_map = input
        ...
        .map(|card| {
            let number_regex = Regex::new(r"(\d+)").unwrap();
            let card_id = number_regex.captures(card[0]).unwrap()[0]
                .parse::<i32>()
                .unwrap();
            let winning_numbers = number_regex
                .captures_iter(card[1])
                .map(|cap| cap[0].parse::<i32>().unwrap())
                .collect::<Vec<i32>>();
            let clone_cards = number_regex
                .captures_iter(card[2])
                .map(|cap| cap[0].parse::<i32>().unwrap())
                .filter(|number| winning_numbers.contains(number))
                .scan(card_id, |id, _| {
                    *id += 1;
