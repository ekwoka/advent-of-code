import { folds, input } from './input.mjs';
...
const points = input.map((string) =>
  string.split(',').map((number) => Number.parseInt(number)),
);

let [height, width] = [
  Math.max(...points.map(([_x, y]) => Number.parseInt(y))) + 1,
  Math.max(...points.map(([x, _y]) => Number.parseInt(x))) + 1,
];

height = height % 2 ? height : height + 1;
width = width % 2 ? width : width + 1;

const grid = Array.from(Array(height).values()).map(() =>
  Array(width).fill('.'),
);
...
function foldAndCount(grid, folds, foldCount) {
  ...
  const endGrid = fold(grid, folds.slice(0, foldCount || folds.length));
...
function translateFold(points, folds) {
  ...
  points = folds.reduce((points, instruction) => {
    const [direction, value] = instruction.split(' ').at(-1).split('=');
    return points.map(([x, y]) => {
      if (direction === 'x')
        return [x > value ? Math.abs(x - value * 2) : x, y];
