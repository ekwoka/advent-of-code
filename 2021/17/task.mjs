...
const VALUES = Array(200).fill().map((_, i) => i)

function calcHeight(xV, yV) {
    let trajectory = [xV, yV]
    let position = [...ORIGIN]
    let inTarget = false
    let maxHeight = 0

    while (!(position[0] >= TARGET.x.max || position[1] <= TARGET.y.min) && !inTarget) {
        position[0] += xV
        position[1] += yV
        xV = xV > 0 ? xV - 1 : 0
        yV -= 1
        ...
        if (position[0] >= TARGET.x.min && position[1] <= TARGET.y.max && position[0] <= TARGET.x.max && position[1] >= TARGET.y.min) {
            inTarget = true
        }
    }
