...
const BIN = [...input].reduce((acc, char) => acc.concat(parseInt(char, 16).toString(2).padStart(4, '0')), '');
...
function findValue(object) {
  ...
  try {
    ...
    if ([0, 1].includes(object.type))
      result = data.length >= 2 ? data.reduce((acc, val) => {
...
function addVersions(obj) {
  ...
  while (queue.length) {
    ...
    if (typeof current.content !== 'number') queue.push(...current.content) || queue.push(current.content);
  }
  return result;
}
...
function parseBin(bin) {
  ...
  if (type === 4) {
    ...
    thisContent = thisContent.reduce((acc, chunk) => acc.concat(chunk.substring(1)), '');
    ...
    if (lengthIndicator == 15) {
        ...
        content.push(...parseBin(bin.substring(7 + lengthIndicator, 7 + lengthIndicator + contentLength)));
        result = [{ version, type, content }];
        result.push(...parseBin(bin.substring(7 + lengthIndicator + contentLength)));
    }
