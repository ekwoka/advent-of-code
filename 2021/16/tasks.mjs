...
const BIN = [...input].reduce(
  (acc, char) =>
    acc.concat(Number.parseInt(char, 16).toString(2).padStart(4, '0')),
  '',
);

function task1(bin) {
  console.log('Parsing...');
  const data = parseBin(bin)[0];
...
function parseBin(bin) {
  ...
  const version = Number.parseInt(bin.substring(0, 3), 2);
  const type = Number.parseInt(bin.substring(3, 6), 2);
  let content = [];
  let result;
  if (Number.isNaN(type)) return content;
  if (type === 4) {
    result = [];
    bin = bin.substring(6);
    const chunks = bin.match(/.{1,5}/g);
    const lastChunk = chunks.findIndex((chunk) => chunk[0] === '0');
    let thisContent = chunks.slice(0, lastChunk + 1);
    const remaining = chunks.slice(lastChunk + 1);
    thisContent = thisContent.reduce(
      (acc, chunk) => acc.concat(chunk.substring(1)),
      '',
    );
    content = Number.parseInt(thisContent, 2);
    const nextBin = remaining.join('');
    result.push({ version, type, content });
    if (nextBin.length > 10) result.push(...parseBin(nextBin));
  } else {
    const lengthIndicator = bin.substring(6, 7) === '1' ? 11 : 15;
    if (lengthIndicator === 15) {
      const contentLength = Number.parseInt(
        bin.substring(7, 7 + lengthIndicator),
        2,
      );
      content.push(
        ...parseBin(
    ...
    if (lengthIndicator === 11) {
      ...
      const chunks = parseBin(bin.substring(7 + lengthIndicator));
